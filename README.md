...
- Preview画面は拡大縮小表示できるようにしたい
  - ボタンで拡張・縮小可能、また、マウスやダブルクリックなどで直感的に操作可能にしたい
- PDF、コピー、PNGに関して、現在画面に映っている情報が出力されるが、画面の下や横にスクロールした分が途切れてしまう事象を改修したい
- mermaidのコードに誤りがある時のデバッグがやりにくい
  - エラーメッセージがポップアップ画面風で中身のコピーもできない上にすぐ消えてしまう点を改善したい。
  - エラーメッセージはコピペできるようにしたい。mermaidのコードを修正すると、次のエラーが出てきてデバッグしやすい状態にしたい。

### PNG、PDF、コピー時に画面に表示されている領域以外の画像データが切れてしまう現象あり（調査して修正する）
